// Toggle dropdown visibility (guarded for mobile pages without the control)
const __accountBtn = document.getElementById('accountDropdownBtn');
if (__accountBtn) {
  __accountBtn.addEventListener('click', function (e) {
    e.stopPropagation();
...
document.addEventListener('click', function (e) {
  ...
  if (!dropdown || !accountBtn) return; // nothing to do on mobile if elements are absent
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize the page
    // --- FIX: ensure lessons are loaded before init() ---
    (function waitForLessons() {
    if (window.lessons && Object.keys(window.lessons).length) {
        init();
    } else {
        setTimeout(waitForLessons, 150);
    }
    })();
...
// Initialize the page
function init() {
     // --- SAFETY FIX for mobile: rebind grid if selector mismatch ---
    if (!cardsGrid || !cardsGrid.appendChild) {
        const altGrid = document.querySelector('.cardsGrid, .cardsContainer, #cardsGrid');
        if (altGrid) window.cardsGrid = altGrid;
        else console.warn('⚠ cardsGrid element not found');
    }
...
// --- MOBILE FIX: ensure lessons exist, then render ---
if (!window.__MOBILE_LESSON_FIX__) {
  window.__MOBILE_LESSON_FIX__ = true;
  document.addEventListener("DOMContentLoaded", () => {
    if (!window.lessons || Object.keys(window.lessons).length === 0) {
      const observer = new MutationObserver(() => {
        if (window.lessons && Object.keys(window.lessons).length) {
          try { showLessons(currentCategory); updatePagination(); } catch (e) { console.error(e); }
          observer.disconnect();
        }
      });
