...
$prompt = "Take note that you should not use any bold texts in this prompt. Just generate a plain text (No text effects/decorations). 
            Generate a quiz in {$selectedOption} format with {$numberOfQuestion}
             questions based on the following text:\n\n{$text}
             and include the answer key too, thanks!";
        // . "Generate a quiz in {$selectedOption} format with {$numberOfQuestion} "
        // . "questions based on the following text:\n\n{$text}"
        // . "Please include the answer key too, thanks!";   // <-- use $text

/**
 * 3️⃣  OpenRouter API configuration
 */
$apiKey = "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";
$apiUrl = "https://openrouter.ai/api/v1/chat/completions";   // match variable name
...
$payload = [
    ...
    "messages" => [
        [
            "role" => "system",
            "content" => "You are an assistant that creates quizzes from provided text."
        ],
        [
            "role" => "user",
            "content" => $prompt
...
function normalizeQuizPayload($raw, string $selectedOption, int $limit = 15): array {
    ...
    // Map UI selection to DB enum (tbl_questions.question_type)
    $typeMap = [
        'Multiple Choice'     => 'mcq',
    ...
    foreach ($raw as $q) {
        ...
        // Type (prefer explicit, else from selectedOption)
        $qtypeRaw = strtolower(trim($q['type'] ?? $q['question_type'] ?? ''));
        $qtype = $defaultType;
        foreach ($typeMap as $ui => $dbEnum) {
            if ($qtypeRaw === strtolower($dbEnum) || $qtypeRaw === strtolower($ui)) { $qtype = $dbEnum; break; }
        }
        ...
        if ($qtype === 'true_false') {
            $ansTF = is_string($answer) ? strtolower(trim($answer)) : (is_bool($answer) ? ($answer ? 'true' : 'false') : '');
        ...
        elseif ($qtype === 'mcq' && is_array($choices) && count($choices) > 0) {
            // answer could be index or string
            $answerIndex = is_int($answer) ? $answer : null;
            foreach (array_values($choices) as $idx => $opt) {
                $otext = trim(is_array($opt) ? ($opt['text'] ?? $opt['option'] ?? '') : (string)$opt);
                if ($otext === '') continue;
                $isCorrect = 0;
                if ($answerIndex !== null) {
                    $isCorrect = ($idx === $answerIndex) ? 1 : 0;
                } elseif (is_string($answer)) {
                    $isCorrect = (trim($answer) === $otext) ? 1 : 0;
                } elseif (is_array($opt) && isset($opt['is_correct'])) {
                    $isCorrect = $opt['is_correct'] ? 1 : 0;
                }
                $options[] = ['text' => $otext, 'is_correct' => $isCorrect];
            }
            // Ensure at least one correct (fallback)
            if (!array_filter($options, fn($o) => !empty($o['is_correct']))) {
                if (isset($options[0])) $options[0]['is_correct'] = 1;
            }
        }
        // Identification / Fill in the Blanks: store the correct answer as the single option
        else {
            $ansText = '';
            if (is_string($answer)) $ansText = trim($answer);
            elseif (is_array($answer)) $ansText = trim((string)($answer['text'] ?? $answer[0] ?? ''));
            $ansText = $ansText !== '' ? $ansText : '[answer]';
